...
class MainWindow(QMainWindow):
    def __init__(self, pics_root: Path, theme: ThemeManager) -> None:
        ...
        self.view_single = SingleView(self._library)
        self.view_grid = GridView(self._library)
        self.view_free = FreeView(self._library)
        ...
        if not self._library.entries:
            self.statusBar().showMessage(
                f"未在 {pics_root} 下找到任何图片  ·  请检查目录结构"
            )
    ...
    def _refresh_library(self) -> None:
        ...
        if n == 0:
            QMessageBox.warning(
                self, "扫描结果", f"未在 {self._pics_root} 下找到任何图片"
            )
